...
export const BALLS: readonly BallConfig[] = [
  {
    ...
    dateIso: '2026-10-17T21:00:00-04:00',
    date: 'OCT 17',
    year: '2026',
    day: 'SATURDAY',
    ...
    namePattern: '%Greek Ball%',
  },
  {
    slug: 'monsters-ball',
    chapter: '02',
    brandKey: 'monsters_ball',
    name: 'Monster’s Ball',
    title: 'AFTER DARK',
    dateIso: '2026-10-31T20:00:00-04:00',
    date: 'OCT 31',
    year: '2026',
    day: 'SATURDAY',
  ...
  },
  {
    ...
    dateIso: '2026-11-28T20:00:00-05:00',
    date: 'NOV 28',
    year: '2026',
    day: 'SATURDAY',
  ...
  },
  {
    ...
    name: 'Snow Ball',
    title: 'WINTER WHITE',
    dateIso: '2026-12-12T20:00:00-05:00',
    date: 'DEC 12',
    year: '2026',
    day: 'SATURDAY',
    ...
    namePattern: '%Snow Ball%',
  },
  {
    slug: 'champagne-ball',
    chapter: '05',
    brandKey: 'champagne_ball',
    name: 'Champagne Ball',
    title: 'THE TOAST',
    dateIso: '2027-01-02T21:00:00-05:00',
    date: 'JAN 02',
    year: '2027',
    day: 'SATURDAY',
  ...
  },
  {
    ...
    name: 'Rose Ball',
    title: 'LOVE IN COLOR',
    dateIso: '2027-02-13T21:00:00-05:00',
    date: 'FEB 13',
    year: '2027',
    day: 'SATURDAY',
...
export type BallLiveState = {
  ...
  statusLabel: string;
  tiers: TicketTier[];
  admissionTiers: TicketTier[];
  sectionTiers: TicketTier[];
};
...
export function getBallConfig(slug: string): BallConfig | undefined {
...
export async function getBallLiveState(config: BallConfig): Promise<BallLiveState> {
  const [{ data: eventbrite }, { data: settings }, { data: eventRows }] = await Promise.all([
    supabase
      .from('eventbrite_events')
      .select('eventbrite_url,eventbrite_event_id,event_date,city,venue_name,venue_address,event_time,is_active,khg_tracking_url')
      .eq('brand_key', config.brandKey)
      .eq('is_active', true)
    ...
    supabase
      ...
      .select('status,venue_name,venue_address,city,ticket_url,doors_time,is_active,metadata')
  ...
  const { data: tierRows } = eventIds.length
    ? await supabase
        ...
        .select(
          'khg_event_id,tier_key,tier_label,display_order,price_cents,capacity,is_vip,is_section,section_capacity,bottles_included,description,is_active',
        )
        .in('khg_event_id', eventIds)
        .eq('is_active', true)
  ...
  return {
    ticketUrl,
    trackingUrl,
    eventbriteEventId: eventbrite?.eventbrite_event_id || null,
    venueName: settings?.venue_name || eventbrite?.venue_name || null,
    venueAddress: settings?.venue_address || eventbrite?.venue_address || null,
    city: settings?.city || eventbrite?.city || 'Atlanta',
    eventTime: settings?.doors_time || eventbrite?.event_time || null,
    onSale,
    soldOut,
    statusLabel: soldOut ? 'SOLD OUT' : onSale ? 'TICKETS LIVE' : 'COMING SOON',
    tiers,
    admissionTiers: tiers.filter((tier) => !tier.is_section),
    sectionTiers: tiers.filter((tier) => tier.is_section),
  };
}
