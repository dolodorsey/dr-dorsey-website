...
function toBrandCard(entity: RegistryEntity): BrandCard {
  const primary =
    entity.destinations?.find((destination) => destination.is_primary) ||
...
export function useEnterpriseRegistry(fallback: BrandCard[]) {
  ...
  useEffect(() => {
    ...
    fetch('/api/enterprise/registry?current_focus=true', {
      cache: 'no-store',
  ...
  const brands = useMemo(
    () => (entities.length ? entities.map(toBrandCard) : fallback),
    [entities, fallback],
  );
